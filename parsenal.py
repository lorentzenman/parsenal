...
#TODO change the XML parser to LXML\SAX type parser so that it is more memory efficient
...
def parse_nessus_file_db(nessus_doc, checkfor, db_cursor, all_issues, infoplugins):
	""" Parses nessus file and outputs to sqlite database """
	# TODO : swap out parser
			...
			if all_issues == True:
				if severity_rating > 0 or plugin_name in infoplugins:
					...
					if len(plugin_output) > 0:
						...
						db_cursor.execute("INSERT INTO issues(host_ipaddress, host_name, host_port, plugin_name, plugin_output, severity) VALUES(?,?,?,?,?,?)", (host_name, host_name, host_port, plugin_name, plugin_output, int(severity_rating)))
					else:

						# execute the query without the plugin_output
						db_cursor.execute("INSERT INTO issues(host_ipaddress, host_name, host_port, plugin_name, severity) VALUES(?,?,?,?,?)", (host_name, host_name, host_port, plugin_name, int(severity_rating)))
					...
					if plugin_name in checkfor:
						...
						if len(plugin_output) > 0:
							plugin_output = plugin_output[0].firstChild.nodeValue
							# add this plugin output to the relevant table, formatting of output is kept when populating the table which is cool
							db_cursor.execute("INSERT INTO issues(host_ipaddress, host_name, host_port, plugin_name, plugin_output) VALUES(?,?,?,?,?)", (host_name, host_name, host_port, plugin_name, plugin_output))
						else:
							# execute the query without the plugin_output
							db_cursor.execute("INSERT INTO issues(host_ipaddress, host_name, host_port, plugin_name) VALUES(?,?,?,?)", (host_name, host_name, host_port, plugin_name))
